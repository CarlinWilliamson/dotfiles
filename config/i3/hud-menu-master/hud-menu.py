...
def try_appmenu_interface(window_id):
  ...
  if 'dmenu' in sys.argv:
    menu_cmd = subprocess.Popen(['dmenu', '-i'], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
  else:
    menu_cmd = subprocess.Popen(['rofi', '-dmenu', '-i', '-location', '1', '-width', '100', '-scroll-method', '1', '-color-enabled', '-color-window', "#242424, #ffffff", '-color-normal', "#242424, #FFFFFF, #242424, #398ee7", '-separator-style', 'solid', '-p', ''], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
